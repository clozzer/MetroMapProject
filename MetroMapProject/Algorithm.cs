...
namespace MetroMapProject
{
    class Algorithm
    {
        ...
        {
            return (float)Math.Sqrt(area / knoten.Count) / 8;
        }
        ...
        public void algorithm2(int iterations, int width, int length)
        {
            for(int i = 0; i < iterations; i++)
            {
                //abstoßendeKraft aller Knoten an einem unbeweglichen Knoten
                foreach (Knoten k in knoten)
                {
                    if (!k.ismoveable())
                    {
                        //abstoßende Knoten finden
                        List<Knoten> mitkverbunden = new List<Knoten>();
                        foreach (Kante e in kanten)
                        {
                            if (e.getende() == k)
                            {
                                mitkverbunden.Add(e.getstart());
                                e.getstart().addunmoveable(k);
                            }
                            if (e.getstart() == k)
                            {
                                mitkverbunden.Add(e.getende());
                                e.getende().addunmoveable(k);
                            }
                        }

                        //sie sich abstoßen lassen
                        foreach (Knoten v in mitkverbunden)
                        {
                            v.setdisp(new Vector(0, 0));
                            foreach (Knoten u in mitkverbunden)
                            {
                                if (v != u)
                                {
                                    Vector p = v.getpos() - u.getpos();
                ...
                foreach (Knoten v in knoten)
                {
                    if (v.ismoveable())
                    {
                        ...
                        if (!double.IsNaN(normalized.Length))
                        {
                            ...
                            Vector starttoziel = ziel - v.getunmoveable()[0].getpos();
                            v.setpos(v.getunmoveable()[0].getpos());
                            v.setx((int)v.getpos().X);
                            v.sety((int)v.getpos().Y);
                            ...
                            while (eucliddistance(v, v.getunmoveable()[0]) < optimaldistance(width *  length))
                            {
                                ...
                                v.setpos(v.getpos() + tempnormalized);
                                v.setx((int)v.getpos().X);
                                v.sety((int)v.getpos().Y);
                            }
                        }
